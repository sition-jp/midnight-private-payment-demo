/**
 * Deploy private-payment contract to Midnight Preprod network
 */
...
// ─── Private State & Witness Provider ─────────────────────────────────────────
...
/**
 * Witness provider implementing all 11 witnesses for the private-payment contract.
 * Each witness returns [updatedPrivateState, returnValue].
 */
export function createWitnesses(): Witnesses<PrivatePaymentState> {
  return {
    local_secret_key(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, Uint8Array] {
      return [context.privateState, context.privateState.secretKey];
    },

    private_amount(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, bigint] {
      return [context.privateState, context.privateState.pendingAmount];
    },

    private_recipient(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, Uint8Array] {
      return [context.privateState, context.privateState.pendingRecipient];
    },

    get_balance(context: WitnessContext<Ledger, PrivatePaymentState>, pk: Uint8Array): [PrivatePaymentState, bigint] {
      const balance = context.privateState.balances.get(toHexKey(pk)) ?? 0n;
      return [context.privateState, balance];
    },

    get_salt(context: WitnessContext<Ledger, PrivatePaymentState>, pk: Uint8Array): [PrivatePaymentState, Uint8Array] {
      const salt = context.privateState.salts.get(toHexKey(pk)) ?? new Uint8Array(32);
      return [context.privateState, salt];
    },

    new_salt(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, Uint8Array] {
      return [context.privateState, randomBytes32()];
    },

    store_balance(context: WitnessContext<Ledger, PrivatePaymentState>, pk: Uint8Array, balance: bigint, salt: Uint8Array): [PrivatePaymentState, []] {
    ...
    get_recipient_balance(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, bigint] {
      return [context.privateState, context.privateState.recipientBalance];
    },

    get_recipient_salt(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, Uint8Array] {
      return [context.privateState, context.privateState.recipientSalt];
    },

    new_recipient_salt(context: WitnessContext<Ledger, PrivatePaymentState>): [PrivatePaymentState, Uint8Array] {
      return [context.privateState, randomBytes32()];
    },

    store_recipient_balance(context: WitnessContext<Ledger, PrivatePaymentState>, pk: Uint8Array, balance: bigint, salt: Uint8Array): [PrivatePaymentState, []] {
      const newBalances = new Map(context.privateState.balances);
      const newSalts = new Map(context.privateState.salts);
      newBalances.set(toHexKey(pk), balance);
      newSalts.set(toHexKey(pk), salt);
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║       Deploy private-payment to Midnight Preprod           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        // Generate a deterministic secret key from the wallet seed for the contract
        const contractSecretKey = crypto.createHash('sha256')
          .update(Buffer.from(seed.trim(), 'hex'))
        ...
        console.log('  [DEBUG] Error cause:', errCause);
        console.log('  [DEBUG] Full error:', fullError);
        if (err?.stack) console.log('  [DEBUG] Stack:', err.stack.split('\n').slice(0, 5).join('\n'));
        console.log('');
        ...
        if (fullError.includes('Not enough Dust')) {
          ...
          if (attempt < MAX_RETRIES) {
            ...
            console.log(`  │      ${String(address).slice(0, 50)}...  │`);
            console.log('  │                                                           │');

...
一般的な公開ブロックチェーンでは、取引データがチェーン上から確認できます。

このデモ契約は **ZKP（ゼロ知識証明）** を使い、金額・残高の実値・saltを公開せずに残高コミットメントを更新します。一方、送信者と受取人の公開鍵はチェーン上に残ります。

```
チェーン上に残る情報 → 送信者・受取人の公開鍵、残高コミットメント、取引メタデータ
公開しない情報       → 送金額、残高の実値、salt
```

この実装には受取人へprivate stateを配送する経路がないため、実用的な二者間決済ではなく、秘匿残高更新の概念実証です。
...
```
Compact言語で、送金額と残高の実値を非公開にし、送信者と受取人の公開鍵を公開する残高更新コントラクトを作成してください。

要件:
- 送金額（amount）は非公開（witness）
- 送信者と受取人の公開鍵は disclose() して公開する
- 残高の正当性はZKPで証明
- 残高不足の場合はエラー
...
- `private_recipient`：witnessから渡されるが、回路内で `disclose()` され、受取人公開鍵としてチェーンに残る

#### circuit（処理ロジック）

| 回路名 | 種別 | 説明 |
|--------|------|------|
| `deposit` | 公開 | 公開額でデモ残高コミットメントを更新（トークン移動なし） |
| `private_transfer` | ZKP | 金額と残高実値を秘匿し、両者の公開鍵を開示してコミットメントを更新 |
| `check_balance` | 公開出力 | 呼び出すと残高の実値を公開する |
